...
function dressAtlas(snap) {
  ...
  if (globeWide() && !globeMoving) {
    tellAtlas({ rotmg: 'bare', on: false, snap: false });
    return;
  }
...
/* What share of the window the world should fill for the ring to sit round
   it. Also where the world's own patch is sized, since the two are the same
   number and reading it is what keeps them the same. */
function worldShare() {
...
function set(want, remember) {
  ...
  on = want;
  /*
   * Written every time, not only on a change. The whole of the ring's layout
   * hangs off this one attribute - the cards hidden, the atlas promoted from
   * a panel to the page, the corner row moved - so if it ever drifted from
   * `on`, what you would get is the ordinary front page with the ring's
   * layout still on it. Setting it again when it already says the right thing
   * costs nothing and takes that whole class of breakage away.
   */
