...
function v314EnsureStyles(){
  ...
  style.textContent=[
    ...
    '#v314SelectedIndex .v314-index-head{display:flex;align-items:center;gap:10px;margin:0 0 10px;color:#b9c7dd;font-size:11px;font-weight:800;letter-spacing:.12em;text-transform:uppercase}',
    '#v314SelectedIndex .v314-index-head:after{content:"";height:1px;flex:1;background:#364052}',
    '#v314SelectedIndex .v314-index-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}',
    '#v314SelectedIndex .v314-index-card{min-width:0;border:1px solid #3b4558;border-radius:8px;background:#202632;padding:10px;box-shadow:0 5px 16px rgba(0,0,0,.15)}',
    '#v314SelectedIndex .v314-index-top{display:grid;grid-template-columns:48px minmax(0,1fr);gap:9px;align-items:center}',
    '#v314SelectedIndex .v314-index-art{width:48px;height:48px;border:1px solid #3a4455;border-radius:6px;background:#171c24;display:grid;place-items:center;overflow:hidden}',
    '#v314SelectedIndex .v314-index-art .thumb,#v314SelectedIndex .v314-index-art canvas{max-width:46px;max-height:46px}',
    '#v314SelectedIndex .v314-index-empty-art{font-size:24px;color:#6f7b8f}',
    '#v314SelectedIndex .v314-index-kind{font-size:10px;color:#8fa1bd;text-transform:uppercase;letter-spacing:.08em;font-weight:800}',
    '#v314SelectedIndex .v314-index-name{margin-top:2px;color:#f5f7fb;font-size:13px;font-weight:800;line-height:1.15;overflow-wrap:anywhere}',
    '#v314SelectedIndex .v314-index-sub{margin-top:3px;color:#9daac0;font-size:10px;line-height:1.25}',
    '#v314SelectedIndex .v314-index-actions{display:flex;flex-wrap:wrap;gap:6px;margin-top:9px}',
    '#v314SelectedIndex .v314-index-open{display:inline-flex;align-items:center;min-height:27px;padding:0 8px;border:1px solid #6686bd;border-radius:5px;background:#35517f;color:#fff;text-decoration:none;font-size:11px;font-weight:800}',
    '#v314SelectedIndex .v314-index-open:hover{filter:brightness(1.12)}',
    '#v314SelectedIndex .v314-index-missing{font-size:10px;color:#8f9bae;line-height:1.25}',
    '#v314SelectedIndex .v314-index-row{display:grid;grid-template-columns:48px minmax(0,1fr);gap:6px;margin-top:8px;padding-top:7px;border-top:1px solid #313a49;font-size:10px}',
    '#v314SelectedIndex .v314-index-row>span:first-child{color:#8796ad;font-weight:800}',
    '#v314SelectedIndex .v314-index-chips{display:flex;flex-wrap:wrap;gap:4px;min-width:0}',
    '#v314SelectedIndex .v314-index-chip{display:inline-flex;max-width:100%;padding:2px 5px;border:1px solid #3c485c;border-radius:999px;background:#1a202a;color:#c9d4e5;text-decoration:none;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
    '#v314SelectedIndex a.v314-index-chip:hover{border-color:#6686bd;color:#fff}',
...
function v315EnsureStyles(){
  ...
  style.textContent=[
    '#v315SandboxBar{box-sizing:border-box;display:flex;flex-wrap:wrap;align-items:center;gap:8px 12px;margin:8px auto 4px;padding:8px 10px;border:1px solid #364154;border-radius:7px;background:#171c24;color:#c9d3e3;font-size:11px}',
    '#v315SandboxBar .v315-help{flex:1 1 245px;min-width:190px;color:#aebbd0;white-space:normal}',
    '#v315SandboxBar .v315-help b{color:#eef3fb;font-weight:800}',
    '#v315SandboxBar .v315-speed{display:flex;align-items:center;gap:7px;white-space:nowrap;color:#aebbd0}',
    '#v315SandboxBar .v315-speed strong{color:#eef3fb}',
    '#v315SandboxBar .v315-speed input{width:112px;accent-color:#7598d3}',
    '#v315SandboxBar .v315-speed output{min-width:82px;color:#c7d5eb;font-variant-numeric:tabular-nums}',
    '#v315SandboxBar .v315-combos{display:flex;align-items:center;gap:6px;margin-left:auto;flex-wrap:wrap}',
    '#v315SandboxBar button{min-height:28px;padding:0 9px;border:1px solid #455269;border-radius:5px;background:#252d3b;color:#e5ebf5;font:inherit;font-weight:700;cursor:pointer}',
    '#v315SandboxBar button:hover{border-color:#718dbb;background:#2e3a4e}',
    '#v315SandboxBar button.is-on{border-color:#7397d3;background:#3b5685;color:white}',
    '#v315SandboxBar .v315-save{border-color:#6b88b7;background:#31496f}',
    '#v315SandboxBar .v315-count{display:inline-flex;align-items:center;justify-content:center;min-width:18px;height:18px;padding:0 4px;border-radius:999px;background:#171c24;color:#b9c8df;font-size:10px}',
    '.combo-dialog-backdrop{position:fixed;inset:0;z-index:10000;display:grid;place-items:center;padding:20px;background:rgba(6,8,12,.72);box-sizing:border-box}',
    '.combo-dialog{width:min(92vw,390px);padding:18px;border:1px solid #4b5970;border-radius:10px;background:#1b212b;color:#e8edf6;box-shadow:0 18px 55px rgba(0,0,0,.48);box-sizing:border-box}',
    '.combo-dialog h2{margin:0 0 14px;font-size:18px}',
    '.combo-dialog label{display:grid;gap:6px;color:#aebbd0;font-size:12px;font-weight:700}',
    '.combo-dialog input{box-sizing:border-box;width:100%;height:38px;padding:0 10px;border:1px solid #46546a;border-radius:6px;outline:none;background:#11161e;color:#f4f7fb;font:14px system-ui,sans-serif}',
    '.combo-dialog input:focus{border-color:#7598d3;box-shadow:0 0 0 3px rgba(117,152,211,.18)}',
    '.combo-dialog-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:16px}',
    '.combo-dialog-actions button{min-height:34px;padding:0 13px;border:1px solid #46546a;border-radius:6px;background:#252d3b;color:#e5ebf5;font:12px system-ui,sans-serif;font-weight:800;cursor:pointer}',
    '.combo-dialog-actions .combo-dialog-save{border-color:#6b88b7;background:#35517f;color:#fff}',
