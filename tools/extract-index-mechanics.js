'use strict';
// Mechanical extraction moved unchanged from build-theorycraft.js.
// Only build-index supplies source text; this module never opens source files.
const fs = require('fs'), path = require('path');
module.exports = function extractMechanics({root, byType, byName, documents}) {
/*
...
/* ---------------- the enchantments ---------------- */
const enchantText = documents.get('Enchantments.xml');
const enchants = [];
...
const sets = [];
{
  const raw = documents.get('EquipmentSets.xml');
  if (raw) {
...
return { classes, items, enchants, sets, bosses, itemOf, shotOf, wornOf };
};
